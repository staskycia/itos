...
from app.models import File, FileStatus, Post, PostStatus, Tag, User, UserRole
...
def home():
    ...
    query = Post.query.filter_by(status=PostStatus.visible).order_by(
    ...
    resp = make_response(
        render_template(
            "board.html", pagination=pagination, tagid=tagid, last_read=last_read
        )
    )
...
def userfile(filename):
    ...
    if file.status != FileStatus.visible and not (
